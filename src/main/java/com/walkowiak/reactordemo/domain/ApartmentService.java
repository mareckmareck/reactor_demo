...
class ApartmentService {
    ...
    Collection<Apartment> getApartments() {
        Mono<Collection<Apartment>> bigCityAparts = bigCityApartsClient.getApartments();
        Mono<Collection<Apartment>> cozyHousesAparts = cozyHousesClient.getApartments();

        Mono<Collection<Apartment>> allAparts = bigCityAparts.zipWith(cozyHousesAparts, CollectionUtils::union);

        logger.info("Starting execution");
        StopWatch stopWatch = new StopWatch();
        stopWatch.start();

        Collection<Apartment> result = allAparts.block();
